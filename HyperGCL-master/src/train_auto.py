    ...
    def print_statistics(self, run=None):
        if run is not None:
            ...
            try:
                ...
                try:
                    y_true = FINAL_DATA.y[FINAL_SPLIT['test']].detach().cpu().numpy()
                    y_pred = FINAL_OUT[FINAL_SPLIT['test']].argmax(dim=1).detach().cpu().numpy()
                    ...
                    print("\n===== CLASS 18 ANALYSIS =====")
                    print("True class 18:", (y_true == 18).sum())
                    print("Predicted class 18:", (y_pred == 18).sum())
                    print("Correct class 18:", ((y_true == 18) & (y_pred == 18)).sum())

                    from sklearn.metrics import confusion_matrix
                    cm = confusion_matrix(y_true, y_pred)

                    print("\nCLASS 18 CONFUSION ROW:")
                    print(cm[18])
                except Exception as e:
                    print("classification report error:", e)
            except Exception as e:
                print("Metric print error:", e)
...
def evaluate(model, data, split_idx, eval_func, result=None):
    if result is not None:
        ...
        print("\n===== TRAIN/EVAL COMPARE =====")
        print("train argmax:")
        print(torch.bincount(out_train.argmax(dim=1).cpu()))

        print("eval argmax:")
        print(torch.bincount(out_eval.argmax(dim=1).cpu()))

        out = F.log_softmax(out_eval, dim=1)
...
def eval_acc(y_true, y_pred):
    ...
    y_pred_np = y_pred.argmax(dim=-1).detach().cpu().numpy()

    is_labeled = y_true_np == y_true_np

    y_true_np = y_true_np[is_labeled]
    y_pred_np = y_pred_np[is_labeled]
...
def permute_edges(data, aug_ratio, permute_self_edge):
    ...
    #     
        ...
        print("REMOVE_LEN =", len(edge2remove_index))
        print("NUM_HYPEREDGES =", data.num_hyperedges[0].item())
        print("EDGE1_MIN =", edge_index[1].min())
        print("EDGE1_MAX =", edge_index[1].max())
    ...
    if args.add_e:
        edge_index = np.concatenate((edge_after_remove, idx_add), axis=1)
    else:
...
def create_hypersubgraph(data, args):
    ...
    sub_size = min(args.sub_size, node_size)
    ...
    print("hyperedge_size =", hyperedge_size)
    print("sub_nodes min =", sub_nodes.min().item())
    print("sub_nodes max =", sub_nodes.max().item())
    print("allocated size =", 2*node_size + hyperedge_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--diff_w', type=float, default=0.1)
    ...
    existing_dataset = ['20newsW100', 'ModelNet40', 'zoo',
                        ...
                        'cora', 'citeseer', 'pubmed', 'MIntRec2.0','MIntRec','MELD_DA']
    ...
    if args.dname in existing_dataset:
        ...
        if (f_noise is not None) and dname in synthetic_list:
            ...
            elif dname in ['MIntRec','MELD_DA','MIntRec2.0']:
                p2raw = '../data/MIntRec/'
            elif dname in ['yelp']:
        ...
        elif args.modality == 'tv':
            data.x[:,1536:] = 0

        elif args.modality == 'ta':
            data.x[:,768:1536] = 0

        elif args.modality == 'va':
            data.x[:,:768] = 0

        print("MODALITY =", args.modality)
        print("FEATURE_SUM =", float(data.x.abs().sum()))
        ...
        if args.dname in ['MIntRec','MIntRec2.0','MELD_DA']:
            data.num_hyperedges = torch.tensor(
                [data.edge_index[1].max()-data.n_x[0]+1])
        else:
            data.num_hyperedges = torch.tensor(
                [data.edge_index[0].max()-data.n_x[0]+1])
    
    if args.method in ['AllSetTransformer', 'AllDeepSets']:
        if args.dname not in ['MIntRec','MELD_DA','MIntRec2.0']:
    ...
    for run in range(args.runs):
        ...
        if run == 0:
            print("\n===== TRAIN COUNTS =====")
            print(torch.bincount(data.y[split_idx["train"]].cpu()))

            print("\n===== VALID COUNTS =====")
            print(torch.bincount(data.y[split_idx["valid"]].cpu()))

            print("\n===== TEST COUNTS =====")
            print(torch.bincount(data.y[split_idx["test"]].cpu()))
    ...
    print("\n===== MODEL PARAMETERS =====")
    print("Backbone Parameters:", num_params)
    ...
    if args.mode=="JSD":
        ...
        raise ValueError("Not Support this contrastive manner.")
    for run in tqdm(range(args.runs)):
        ...
        gen_params = count_parameters(view_generator)
        print("Generator Parameters:", gen_params)
        print("Total Parameters:", num_params + gen_params)

        optimizer_g = torch.optim.Adam(view_generator.parameters(), lr=args.g_lr, weight_decay=0)
        best_val = float('-inf')
        for epoch in range(args.epochs):
            # cl loss
            data_sub = create_hypersubgraph(copy.deepcopy(data_pre), args)
            ...
            if epoch == 0:
                print("\n===== INDEX DEBUG =====")
                print("x shape =", data_aug1.x.shape)
                print("edge_index shape =", data_aug1.edge_index.shape)
                print("row max =", data_aug1.edge_index[0].max().item())
                print("row min =", data_aug1.edge_index[0].min().item())
                print("col max =", data_aug1.edge_index[1].max().item())
                print("col min =", data_aug1.edge_index[1].min().item())
                print("n_x =", data_aug1.n_x)
                print("num_hyperedges =", data_aug1.num_hyperedges)
            ...
            with torch.no_grad():
                _, _, aug_weight1, _ = view_generator.generate_only(data_2)

            # ===== Diffusion View 2 =====
            data_3 = copy.deepcopy(data_sub).to(device)

            with torch.no_grad():
                _, _, aug_weight2, _ = view_generator.generate_only(data_3)

            out = model.forward_cl(
                copy.deepcopy(data_sub).to(device),
                aug_weight1
            )

            out_aug = model.forward_cl(
                copy.deepcopy(data_sub).to(device),
                aug_weight2
            )
            ...
            loss_diff = loss_vhgae.detach()

            if args.m_l:
                loss_cl_nat = torch.tensor(
                    0.0,
                    device=device
                )
            if args.linear:
            ...
            if epoch == 0:
                ...
                print("first row:", out[0].detach().cpu())
                print("pred counts:", torch.bincount(out.argmax(dim=1).cpu()))

                print("\n===== EMBEDDING CHECK =====")
                print("logits mean =", out.mean().item())
                print("logits std  =", out.std().item())
                print("logits min  =", out.min().item())
                print("logits max  =", out.max().item())

                pred = out.argmax(dim=1)
                print("unique predicted classes =", torch.unique(pred))
                print("num predicted classes =", len(torch.unique(pred)))

                probs = torch.softmax(out, dim=1)
                print("prob mean =", probs.mean().item())
                print("prob std  =", probs.std().item())
            ...
            if args.use_diff:
                loss += args.diff_w * loss_diff
            # #         Training part
            if args.m_l:
                loss += args.m_l*loss_cl_nat
            print(f"Epoch={epoch} Loss={loss.item():.6f} CL={loss_cl.item():.6f}")
    ...
    #         Evaluation part
            if args.linear:
                ...
                if epoch == 0:
                    print("\n===== DATA DEBUG BEFORE EVAL =====")
                    print("edge min:", data.edge_index[1].min().item())
                    print("edge max:", data.edge_index[1].max().item())
                    print("edge shape:", data.edge_index.shape)
                    print("x sum:", float(data.x.sum()))

                result = evaluate(model, data, split_idx, eval_func, out)

                if epoch == 0:
                    print("\n===== OUT DEBUG =====")
                    print("result[6] shape:", result[6].shape)
                    print("first logits:")
                    print(result[6][0].detach().cpu())
                ...
                if epoch == 0:
                    ...
                    print("train_acc =", result[0])
                    print("valid_acc =", result[1])
                    print("test_acc =", result[2])
                    pred = result[6].argmax(dim=1).cpu()
                    print("eval pred counts:")
            ...
            y_true_run = data.y[split_idx['test']].detach().cpu().numpy()
            y_pred_run = result[6][split_idx['test']].argmax(dim=1).detach().cpu().numpy()
            ...
            save_dir = f"class_reports_{args.dname}"
            os.makedirs(save_dir, exist_ok=True)

            pd.DataFrame(report).transpose().to_csv(
                f"{save_dir}/run_{run+1}.csv"
            )

            global FINAL_OUT, FINAL_SPLIT, FINAL_DATA
            FINAL_OUT = result[6]
            FINAL_SPLIT = split_idx
            FINAL_DATA = data
            # with open("auto_{}_{}.csv".format(args.dname, args.m_l),"a+") as f:
        ...
        # logger.print_statistics(run) #TODO
    
    ### Save results ###
    avg_time, std_time = np.mean(runtime_list), np.std(runtime_list)

    best_val, best_test = logger.print_statistics()
    print("BEST_VAL_VECTOR =", best_val.tolist())

    print("BEST_TEST_VECTOR =", best_test.tolist())
    ...
    with open(filename, 'a+') as write_obj:
        if args.m_l:
            cur_line = f'{args.method}_auto_{args.step}_{args.a_l}_{args.m_l}_{args.lr}_{args.wd}_{args.sub_size}_{args.heads}_ratio_{str(args.aug_ratio)}_t_{str(args.t)}_plr_{str(args.p_lr)}_pepoch_{str(args.p_epochs)}_player_{str(args.p_layer)}_phidden_{str(args.p_hidden)}_drop_{str(args.dropout)}_train_{str(args.train_prop)}'
            if args.add_e:
                cur_line+="_add_e"
        else:
            cur_line = f'{args.method}_{args.lr}_{args.wd}_{args.heads}_{str(args.dropout)}_train_{str(args.train_prop)}'+f'mode'
