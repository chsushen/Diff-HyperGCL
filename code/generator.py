...
class vhgae(torch.nn.Module):
    ...
    def generate(self, data, reward=None):
        ...
        loss_diff, node_gen, edge_gen = self.diffusion.generate(
            x_node,
            x_he
        )

        x_src = node_gen[src] + edge_gen[dst]
        x_dst = node_gen[dst] + edge_gen[src] 

        aug_keep_prob = self.diffusion.generate_edge_prob(
            x_src,
            x_dst
        )
